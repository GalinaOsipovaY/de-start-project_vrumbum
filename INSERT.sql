...
select distinct (SPLIT_PART(SPLIT_PART(auto, ',', 1), ' ', 2) || ' ' || SPLIT_PART(SPLIT_PART(auto, ',', 1), ' ', 3)) as model_name,
                gasoline_consumption
from raw_data.sales;
...
insert into car_shop.sales (id, brand_id, model_id, color_id, price, date, person_id, discount, brand_origin_id)
select s.id, b.brand_id, m.model_id, c.color_id, s.price, date , p.person_id, s.discount, bo.brand_origin_id
from raw_data.sales s
left join car_shop.brands b on b.brand_name = SPLIT_PART(s.auto, ' ', 1) 
...
left join car_shop.brand_origin bo on coalesce(bo.brand_origin_name, 'нет') = coalesce(s.brand_origin, 'нет')
left join car_shop.colors c on c.color_name = SPLIT_PART(s.auto, ',', 2)
left join car_shop.persons p on p.person_name = s.person_name
                             and p.phone = s.phone;